...
def launch_single_gpu(n_rows):
    # Create a folder for each gpu
    gpu_dir = os.path.join(base_dir, "data_weak/base_gpu_1")
    os.makedirs(gpu_dir, exist_ok=True)
    
    # Create a folder for each batch_size
    batch_size_dir = os.path.join(gpu_dir, "nrows_" + str(n_rows))
    ...
    formatted_content = file_content.replace('$$bs_size_key$$', str(max_local_batch))
    formatted_content = formatted_content.replace('$$n_rows$$', str(n_rows))
    ...
    formatted_content = file_content.replace('$$n_gpus$$', str(1))
...
for gpu in n_gpus:
    # Create a folder for each gpu
    gpu_dir = os.path.join(base_dir, "data_weak/gpu_" + str(gpu))
    os.makedirs(gpu_dir, exist_ok=True)
    batch_size = max_local_batch * gpu
    
    # Create a folder for each batch_size
    batch_size_dir = os.path.join(gpu_dir, "nrows_" + str(batch_size))
    os.makedirs(batch_size_dir, exist_ok=True)
    ...
    formatted_content = formatted_content.replace('$$n_rows$$', str(batch_size))
